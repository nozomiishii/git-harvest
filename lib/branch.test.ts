...
test('shouldDeleteBranch never deletes the base branch', () => {
  expect(
    shouldDeleteBranch(
      { checkedOutInSurviving: false, classification: 'merged', isBase: true, isCurrentHead: false, name: 'main' },
      defaultFlags(),
    ),
  ).toBe(false);
});
...
test('shouldDeleteBranch never deletes the current HEAD', () => {
  expect(
    shouldDeleteBranch(
      { checkedOutInSurviving: false, classification: 'merged', isBase: false, isCurrentHead: true, name: 'cur' },
      defaultFlags(),
    ),
  ).toBe(false);
});
...
test('shouldDeleteBranch never deletes a branch checked out in a surviving worktree', () => {
  expect(
    shouldDeleteBranch(
      { checkedOutInSurviving: true, classification: 'merged', isBase: false, isCurrentHead: false, name: 'wt' },
      defaultFlags(),
    ),
  ).toBe(false);
});
...
test('shouldDeleteBranch deletes merged by default and other only at committed threshold', () => {
  const merged = { checkedOutInSurviving: false, classification: 'merged' as const, isBase: false, isCurrentHead: false, name: 'm' };
  const other = { checkedOutInSurviving: false, classification: 'other' as const, isBase: false, isCurrentHead: false, name: 'o' };
...
test('cleanupBranches deletes an untouched branch by default', async () => {
  ...
  expect(result.results.some((r) => r.name === 'br-untouched' && r.action === 'removed')).toBe(true);
...
test('cleanupBranches keeps an other branch by default and deletes it at committed threshold', async () => {
  ...
  const removed = await cleanupBranches('main', defaultFlags({ branch: 'committed' }), worktreePaths(repo), repo);
...
test('cleanupBranches dry-run reports would-remove without deleting', async () => {
  ...
  const result = await cleanupBranches('main', defaultFlags({ dryRun: true }), worktreePaths(repo), repo);
