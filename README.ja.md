...
各リソースは最もリスクの高い（手前の）段階で分類します。未コミットの変更は branch の commit 状態より優先します。フラグはその段階以降（より安全な側）を全部消し、merge済 が default です。merge済 は復旧可能なので安全だからです。

未着手（独自コミットなし・base と同一）と detached（branch を持たない worktree）は この ladder の外です。default では保護し、専用フラグか `--yolo` でのみ削除します。
...
default は段階で判定します（全 scope の閾値 merge済）。フラグは閾値を下げ、残すノードを削除へ切り替えます。各残すノードに「どのフラグで消えるか」を併記します。invariant はどのフラグでも動かせません。detached / untouched は SAFETY ladder の外なので専用フラグ（または `--yolo`）でのみ削除します。
