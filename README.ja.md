...
`--committed` と `--files-changed` は scope で対象を絞れます (`=worktree` / `=claude-worktree` / `=codex-worktree` / `=branch`)。詳しくは scope セクション参照。
...
| `codex-worktree` | `$CODEX_HOME/worktrees` 配下の worktree (通常は `~/.codex/worktrees`) |
| `branch` | ブランチ |
...
Codex app が管理する worktree は `$CODEX_HOME/worktrees` (通常は `~/.codex/worktrees`) 配下に作られます。その worktree は `codex-worktree` scope として扱われ、通常の worktree と同じステージ閾値で判定されます。`--committed=codex-worktree` のように scope 指定で Codex worktree だけ閾値を下げられます。

Codex が `$CODEX_HOME/process_manager/chat_processes.json` に worktree の active process metadata を記録している場合、その worktree も `session running` として保護します。この Codex process 判定は best-effort です。process manager の metadata だけを読み、`osPid` があれば live pid として扱い、最近更新された entry は active signal として扱います。古い entry は無視し、metadata が無い・読めない場合は通常のステージ判定に戻ります。
