...
- **bypass**: `--all` はこれらの保護を無視し、デフォルトブランチとメインワーキングツリー以外を全削除します。
...
#### (任意) Claude Code session schema の監視

Claude Code 側で session ファイルの schema が変わると、active-session 保護が無言で停止し、worktree が想定外に削除される可能性があります。これを検知するには、cron / launchd / Claude Code の Routine などで以下のようなチェックを週1回走らせてください:
...
git-harvest 本体には**意図的に組み込んでいません**。ツールの責務を harvest に集中させ、通知の届け先 (システム通知 / Slack 等) はユーザーが自由に選べるようにするためです。
